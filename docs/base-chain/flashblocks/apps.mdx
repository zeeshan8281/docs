...
Flashblocks is enabled for developers of Base. There are two ways you can integrate with Flashblocks data. You can either use the WebSocket API to stream real-time block updates, or use the RPC API to query the Flashblocks-aware RPC endpoint.
...
You can connect to the websocket endpoint with any WebSocket library of CLI tool. The endpoints are available at:
